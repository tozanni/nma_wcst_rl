...
def WCST_test(nb_test, path):
    ...
    #ACTIVITIES INITIALISATION (Agent)
    ...
    #LONG TERM COMPONENTS INITIALISATION (Agent)
    ...
    #SHORT TERM COMPONENTS INITIALISATION (Agent)
    ...
    #THRESHOLDS OF ACTIVATION (Agent)
    ...
    #Data history for graphs (Main Loop)
    mem_act_history = [] 
    r1_act_history = []
    r2_act_history = []
    r3_act_history = []
    ...
    #Create reference cards (Environment)
    m_percep = perception(nb_dim, nb_templates, nb_features)
    v_data = []

    #Picking a first random rule to follow (Environment)
    ...
    #START OF TEST (Main Loop)
    while(nbTS<6):
        ## Loop is part of the environment
        ...
        if (input_bool==True):
            ...
            print("New card is ", v_data)
            input_bool = False

        #ACTIVITIES COMPUTING (Agent)
        ...
        # Can this be considered on-line learning or just internal agent state.
        # Short term and Long term updates (Is short term for individual games and LT for multiple games?)
        ...
        #ACTIVITIES HISTORY for graph (Test / Agent)
